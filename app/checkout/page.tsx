...
import { ChevronDown, ChevronUp, ChevronRight, CreditCard, DollarSign, ArrowRight } from 'lucide-react'
              ...
              <div className="space-y-2 sm:space-y-3 mb-3 sm:mb-4 pb-3 sm:pb-4 border-b">
                {cartItems.map((item) => (
                  <div key={item.id} className="flex items-start justify-between text-xs sm:text-sm">
                    ...
                    <span className="font-bold text-gray-900 flex-shrink-0">৳{item.price.toFixed(3)}</span>
                  </div>
                ))}
              </div>
            )}
